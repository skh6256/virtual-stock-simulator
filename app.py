...
def ensure_state() -> None:
    if "initial_capital_text" not in st.session_state:
        st.session_state.initial_capital_text = "100,000,000"

    if "initial_price_text" not in st.session_state:
        st.session_state.initial_price_text = "200,000"

    if "buy_amount_text" not in st.session_state:
        st.session_state.buy_amount_text = "20,000,000"

    if "sell_amount_text" not in st.session_state:
...
show_metric(
    top1[1],
    "현재 주가",
    format_won(st.session_state.price),
)
...
show_metric(
    top1[3],
    "보유 현금",
    format_won(st.session_state.cash),
)
...
show_metric(
    top2[0],
    "평균 매수가",
    format_won(st.session_state.avg_price),
)

show_metric(
    top2[1],
    "주식 평가금액",
    format_won(values["market_value"]),
)

show_metric(
    top2[2],
    "총자산",
    format_won(values["total_assets"]),
)

show_metric(
    top2[3],
    "누적 수익금",
    format_won(values["total_pnl"]),
)
...
control_col, buy_col, sell_col = st.columns(
    [1, 1.4, 1.4]
)
